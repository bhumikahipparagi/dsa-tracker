        ...
        <div className="flex-1 min-w-0">
          ...
          {note.source && (
            <a href={note.source} target="_blank" rel="noopener noreferrer"
              className="text-xs text-slate-500 hover:text-orange-400 flex items-center gap-1 mt-0.5">
...
export default function NotesPage() {
  ...
  const FormPanel = () => (
    <div className="fixed inset-0 bg-black/60 flex items-start justify-center z-50 p-4 pt-16 overflow-auto">
      <div className="bg-[#1e1e1e] border border-[#333] rounded-xl w-full max-w-2xl p-5 space-y-4">
        ...
        <select value={form.pattern} onChange={(e) => setForm({ ...form, pattern: e.target.value })}
          className="w-full bg-[#151515] border border-[#333] rounded-lg px-3 py-2 text-sm text-slate-300 outline-none focus:border-orange-500">
        ...
        <textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })}
          ...
          className="w-full bg-[#151515] border border-[#333] rounded-lg px-3 py-2 text-sm text-slate-200 placeholder-slate-600 outline-none focus:border-orange-500 resize-none font-mono" />
        <div className="flex gap-3">
          ...
          <button onClick={closeForm}
            className="px-4 bg-[#151515] border border-[#333] text-slate-400 rounded-lg text-sm hover:text-white transition-colors">
            Cancel
          </button>
        </div>
      </div>
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <button onClick={() => setFilterPattern('')}
          className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
            !filterPattern ? 'bg-orange-500/20 border-orange-500 text-orange-400' : 'border-[#333] text-slate-500 hover:border-[#555]'
          }`}>All</button>
        {NEETCODE_PATTERNS.filter((p) => conceptNotes.some((n) => n.pattern === p)).map((p) => (
          <button key={p} onClick={() => setFilterPattern(p)}
            className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
              filterPattern === p ? 'bg-blue-500/20 border-blue-500 text-blue-400' : 'border-[#333] text-slate-500 hover:border-[#555]'
            }`}>{p}</button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-xl p-12 text-center">
          ...
          <button onClick={openCreate}
            className="mt-4 text-sm text-orange-400 hover:text-orange-300">Add your first note →</button>
        </div>
      ) : (
